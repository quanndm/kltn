...
def get_liver_mask_bbox_v1(source, model_stage_1=None, device=None):
    dataset = get_full_dataset_lits(source, normalizations="minmax", mode="all", device=device)
    liver_masks_bbox = []
    patients_id = []
    sources = []
...
def convert_to_2D_dataset_v1(source, bbox, slides = 3, stride = 2, save_dir = "/content/2D_dataset"):
    """
...
def convert_to_2D_dataset(source, bbox, slides = 3, stride = 2, save_dir = "/content/2D_dataset"):
    ...
    for i in range(len(volume_files)):
        ...
        # Get the liver ROI from the image and segmentation
        bb = next((item["bbox"] for item in bbox if item["patient_id"] == int(patient_id) and item["source"] == source), None)
...
def get_datasets_2d_v1(source_folder, seed, fold_number=5, normalizations="zscores"):
    """
    Get the datasets for the LiTS dataset.
    The function will return the training and testing datasets based on the fold number.
    The datasets are created using the Lits class from the lits module.
